...
//	float *m_matrix;
//public:
//	Matrix(int cols, int rows, float *array);
...
template<class T> std::ostream& operator<<(std::ostream &flux,
		const Matrix<T> &m);
...
Matrix<T> Matrix<T>::subtract(const Matrix &m) const {
	if (!(height == m.height && width == m.width))
		throw std::invalid_argument("Matrix dimension must be the same. L185");
...
Matrix<T> Matrix<T>::subMat(int startH, int startW, int h, int w) const {
	if (!(startH >= 0 && startH + h <= height && startW >= 0
			&& startW + w <= width))
		throw std::invalid_argument("Index out of bounds");
...
void Matrix<T>::print(std::ostream &flux) const {
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			flux << array[i][j];
			ss << array[i][j];
			for (size_t k = 0; k < maxLength[j] - ss.str().size() + 1; k++) {
				flux << " ";
			}
			ss.str(std::string());
		}
