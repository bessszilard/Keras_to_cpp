...
print(arch["config"])
first_layer = arch["config"]["layers"][0]["config"]#arch["config"][0]["config"]
print(first_layer)
# input_shape = first_layer["batch_input_shape"]
input_shape = arch["config"]["build_input_shape"]
input_shape = (input_shape[1], input_shape[2])
print ("Input shape of your network", input_shape)

print ("Generate random input for testing purposes")
random_input = np.random.rand(1, input_shape[0], input_shape[1])
...
with open(args.data_sample, "w") as fin:
    ...
    print ("Save to", args.data_sample, "sample shape", str(input_shape[0]) + " " + str(input_shape[1]) + " ")
    fin.write(str(input_shape[0]) + " " + str(input_shape[1]) + " " + "\n")
    a = random_input[0,0]
    for b in a:
        fin.write(str(b)+'\n')
